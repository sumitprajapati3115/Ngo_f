import { B as getRangeValue, D as calcPositionOrRandomFromSizeRanged, E as calcPositionOrRandomFromSize, I as getRandom, M as getDistance, N as getDistances, Z as Vector, at as millisecondsToSeconds, h as isPointInside, it as half, nt as doublePI, ot as originPoint } from "./browser-D9dcGsUv.js";
import { c as getStyleFromRgb, f as rangeColorToRgb } from "./ColorUtils-tUxAg-oS.js";
import { t as RotateDirection } from "./RotateDirection-D96AavKe.js";
import { t as Absorber } from "./Absorber-DiyeVqKY.js";
//#region node_modules/@tsparticles/plugin-absorbers/browser/AbsorberInstance.js
var absorbFactor = .033, minOrbitLength = 0, minRadius = 0, minMass = 0, minAngle = 0, maxAngle = doublePI, angleIncrementFactor = 1 / 360, defaultLifeDelay = 0, minLifeCount = 0, defaultSpawnDelay = 0, defaultLifeCount = -1;
var AbsorberInstance = class {
	...
	attract(particle, delta) {
		...
		} else this.#updateParticlePosition(particle, delta, v);
	}
	draw(context) {
	...
	#prepareToDie() {
		...
		if ((this.#lifeCount > minLifeCount || this.#immortal) && duration !== void 0 && duration > 0) this.#duration = duration * millisecondsToSeconds;
	}
	#updateParticlePosition(particle, delta, v) {
		...
		if (this.options.orbits) {
			...
			const maxSize = particle.size.max, sizeFactor = particle.options.move.size ? particle.getRadius() / maxSize : 1, deltaFactor = delta.factor || 1, moveSpeed = particle.retina.moveSpeed * sizeFactor * deltaFactor * half;
			particle.position.x = this.position.x + orbitRadius * Math.cos(orbitAngle);
			particle.position.y = this.position.y + orbitRadius * (orbitDirection === RotateDirection.clockwise ? 1 : -1) * Math.sin(orbitAngle);
		...
		} else particle.velocity.addTo(v);
	}
};
//#endregion
export { AbsorberInstance };
